...
const panelTwoParallaxRange = { start: 0, end: 0 };
const PANEL_BACKGROUND_SHIFT_VAR = "--resource-panel-bg-shift";
const PANEL_BACKGROUND_MAX_SHIFT = 80; // px
let backgroundParallaxVideoFraction = 0;
let wheelSnapCleanup = null;
const WHEEL_STEP_THRESHOLD = 120; // 触发一次翻页需要的累计滚轮距离
const WHEEL_LOCK_DURATION = 420; // 每次翻页后的锁定时间（ms）

const resetPanelTwoParallax = () => {
  panelTwoParallaxRange.start = 0;
  panelTwoParallaxRange.end = 0;
  if (panelTwo.value) {
    panelTwo.value.style.removeProperty("--panel-two-bg-offset");
  }
};

const updatePanelTwoParallax = (progress) => {
  const { start, end } = panelTwoParallaxRange;
  if (!panelTwo.value || end <= start) return;
  const ratio = Math.min(Math.max((progress - start) / (end - start), 0), 1);
  const maxOffsetPercent = 3; // 背景最大偏移 3%
  const offset = ratio * maxOffsetPercent;
  panelTwo.value.style.setProperty("--panel-two-bg-offset", `${offset}%`);
};

const getBackgroundParallaxHost = () => {
  if (typeof document === "undefined") return null;
  return document.documentElement;
};

const applyBackgroundParallaxValue = (valuePx) => {
  const host = getBackgroundParallaxHost();
  if (!host) return;
  host.style.setProperty(PANEL_BACKGROUND_SHIFT_VAR, `${valuePx}px`);
};

const resetPanelBackgroundParallax = () => {
  applyBackgroundParallaxValue(0);
};

const updatePanelBackgroundParallax = (progress) => {
  const host = getBackgroundParallaxHost();
  if (!host) return;
  if (progress <= backgroundParallaxVideoFraction) {
    applyBackgroundParallaxValue(0);
    return;
  }
  const horizontalSpan = Math.max(1 - backgroundParallaxVideoFraction, 0.0001);
  const normalized = Math.min(
    Math.max((progress - backgroundParallaxVideoFraction) / horizontalSpan, 0),
    1
  );
  const offset = (normalized * 2 - 1) * PANEL_BACKGROUND_MAX_SHIFT;
  applyBackgroundParallaxValue(offset.toFixed(2));
};

const clearPanelBackgroundParallax = () => {
  const host = getBackgroundParallaxHost();
  if (!host) return;
  host.style.removeProperty(PANEL_BACKGROUND_SHIFT_VAR);
};
...
const isWithinPinnedRange = () => {
  ...
  return scrollPos >= trigger.start && scrollPos <= trigger.end;
};

const resolveSnapPoints = (videoFraction) => {
  const basePoints = new Set([0]);
  if (videoFraction > 0) {
    basePoints.add(videoFraction);
  }
  scrollElementPositions.forEach(({ progress }) => {
    basePoints.add(progress);
  });
  return Array.from(basePoints).sort((a, b) => a - b);
};
...
const scrollToProgress = (progress) => {
  ...
  const target = trigger.start + progress * range;
  window.scrollTo({ top: target, behavior: "smooth" });
};

const setupWheelSnapControl = (videoFraction) => {
  wheelSnapCleanup?.();
  const trigger = scrollAnimation?.scrollTrigger;
  if (!trigger) return;
  const snapPoints = resolveSnapPoints(videoFraction);
  if (snapPoints.length <= 1) return;

  let wheelBucket = 0;
  let currentIndex = snapPoints.reduce((closestIdx, value, idx) => {
    const currentDiff = Math.abs(value - trigger.progress);
    const bestDiff = Math.abs(snapPoints[closestIdx] - trigger.progress);
    return currentDiff < bestDiff ? idx : closestIdx;
  }, 0);
  let isLocked = false;
  let lockTimer = null;
  ...
  const releaseLock = () => {
    isLocked = false;
    wheelBucket = 0;
    lockTimer && window.clearTimeout(lockTimer);
    lockTimer = null;
  };
  ...
  const gotoIndex = (targetIndex) => {
    ...
    isLocked = true;
    scrollToProgress(snapPoints[targetIndex]);
    lockTimer && window.clearTimeout(lockTimer);
    lockTimer = window.setTimeout(releaseLock, WHEEL_LOCK_DURATION);
  };

  const normalizeDelta = (event) => {
    const rawDelta = event.deltaY || event.detail || 0;
  ...
  const onWheel = (event) => {
    ...
    wheelBucket += delta;
    if (Math.abs(wheelBucket) < WHEEL_STEP_THRESHOLD) return;

    const direction = wheelBucket > 0 ? 1 : -1;
    wheelBucket = 0;
...
const calculateScrollElementPositions = (
  ...
  return new Promise((resolve) => {
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        // 收集所有 scroll1-scroll19 元素并计算位置
        const TOTAL_SCROLL_POINTS = 19;
        for (let i = 1; i <= TOTAL_SCROLL_POINTS; i++) {
        ...
        const scroll1 = scrollElementPositions.find(
          (pos) => pos.id === "scroll1"
        );
        const scroll2 = scrollElementPositions.find(
          (pos) => pos.id === "scroll2"
        );
        panelTwoParallaxRange.start = scroll1?.progress ?? 0;
        panelTwoParallaxRange.end =
          scroll2?.progress ?? panelTwoParallaxRange.start;
        resolve();
      });
    });
  });
...
const createScrollTrigger = async () => {
  ...
  backgroundParallaxVideoFraction = videoFraction;
  resetPanelBackgroundParallax();
  ...
  scrollAnimation.scrollTrigger = ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      updatePanelTwoParallax(self.progress);
      updatePanelBackgroundParallax(self.progress);
    },
    snap: {
      snapTo: (value) => {
        // panel01 的视频部分保持吸附效果
  ...
  setupWheelSnapControl(videoFraction);
};
...
.panel-02 {
  background-position: calc(5.2632% + var(--panel-two-bg-offset, 0%)) center;
  transition: background-position 0.6s ease 0.3s;
  display: flex;
  align-items: center;
...
.panel-20 {
  ...
  background-size: cover;
  background-repeat: no-repeat;
  background-position: calc(50% + 1200px)
    center;
